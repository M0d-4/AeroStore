...
struct SourceError: ALTLocalizedError
{
    ...
    var errorFailureReason: String {
        switch self.code
        {
        case .unsupported: return String(format: NSLocalizedString("The source “%@” is not supported by this version of FluxStore.", comment: ""), self.$source.name)
        case .duplicateBundleID:
        ...
        case .blocked:
            let failureReason = String(format: NSLocalizedString("The source “%@” has been blocked by FluxStore for security reasons.", comment: ""), self.$source.name)
            return failureReason
        ...
        case .marketplaceNotSupported:
            let failureReason = String(format: NSLocalizedString("The source “%@” contains notarized apps, which are not supported by this version of FluxStore.", comment: ""), self.$source.name)
            return failureReason
            
        case .marketplaceRequired:
            let failureReason = String(format: NSLocalizedString("One or more apps in source “%@” are missing a marketplaceID. This most likely means they are not notarized, which is not supported by this version of FluxStore.", comment: ""), self.$source.name)
            return failureReason
        }
    }
    ...
    var recoverySuggestion: String? {
        ...
        case .marketplaceRequired:
            let failureReason = String(format: NSLocalizedString("FluxStore can only install marketplace apps that have been notarized by Apple.", comment: ""), self.$source.name)
            return failureReason
