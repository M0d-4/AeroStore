...
private extension AuthenticationOperation
{
    ...
    func fetchCertificate(for team: ALTTeam, session: ALTAppleAPISession, completionHandler: @escaping (Result<ALTCertificate, Swift.Error>) -> Void)
    {
        ...
        {
            let machineName: String = "\(Bundle.main.altAppDisplayName) - \(team.account.firstName)'s \(UIDevice.current.name)"
        ...
        func replaceCertificate(from certificates: [ALTCertificate])
        {
            let ourCertificates = certificates.filter { a in
                a.machineName?.starts(with: "FluxStore") == true || a.machineName?.starts(with: "SideStore") == true || a.machineName?.starts(with: "AltStore") == true
            }
